...
const eventMatchesSearch = (event: any, query: string): boolean => {
...
useHead({
  title: `Calendrier des Événements - ${clubConfig.value?.name || "Club Pongiste Libercourtois"}`,
  meta: [
    {
      name: "description",
      content: `Découvrez le calendrier complet des événements, tournois et compétitions du ${clubConfig.value?.name || "Club Pongiste Libercourtois"}.`,
    },
  ],
});
...
const eventsWithApproachingDeadline = computed(() => {
  ...
  return eventsWithOpenRegistration.value.filter((event) => {
