...
export const useFfttCategories = () => {
  ...
  const decodeFfttCategory = (categoryCode: string): string => {
    ...
    if (match) {
      ...
      const baseName = letter ? FFTT_CATEGORY_MAP[letter] : undefined;
      if (baseName) {
  ...
  const getAgeGroup = (
    ...
    if (
      normalizedCode.startsWith("P") || // Poussin
    ...
    if (
      normalizedCode.startsWith("J") || // Junior
      normalizedCode.startsWith("S") || // Senior
      normalizedCode.startsWith("V")
    ) {
      // Veteran
      return "adult";
    }
