...
import { extractPhaseFromTeamName } from '~/server/utils/teams'
...
export default defineEventHandler(async (_event): Promise<ClubStats> => {
  ...
  try {
    ...
    if (teamsData.success && teamsData.data && teamsData.data.length > 0) {
      // Extract phases and find the current one (highest number = most recent)
      const phases = teamsData.data.map(team => extractPhaseFromTeamName(team.libequipe))
      const currentPhase = Math.max(...phases)
      // Count only teams from current phase
      equipes = teamsData.data.filter(
        team => extractPhaseFromTeamName(team.libequipe) === currentPhase,
      ).length
    }
