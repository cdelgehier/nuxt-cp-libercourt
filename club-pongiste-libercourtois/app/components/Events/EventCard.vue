...
const eventTypeLabel = computed(
  () => getEventTypeConfig(props.event.type).label,
);
const eventTypeBadgeClass = computed(
  () => getEventTypeConfig(props.event.type).class,
);

const canRegister = computed(() => {
  if (!props.event.maxParticipants) return true;
  return (props.event.currentParticipants ?? 0) < props.event.maxParticipants;
});
...
const addToGoogleCalendar = () => {
  try {
    ...
    if (props.event.contact) {
      const contactName =
        typeof props.event.contact === "string"
          ? props.event.contact
